export default{
        ...
        <div class="bottom">
            <nav id="nav">
                <ul>
                    <li 
                        class="col" 
                        v-for="item,idx in headerState.카테고리" 
                        :key="item.메인메뉴"
                        @mouseleave="mouseOutNav"
                    >
                        <a 
                            @mouseenter="mouseOverMainBtn(idx)"  
                            href="#" 
                            :class="['main-btn', {'on':isOn[idx]}]" 
                            :title="item.메인메뉴"
                            @click.prevent="mainEmit(item.메인메뉴)"
                        >
                            {{item.메인메뉴}}
                        </a>
                        <div
    ...
    methods:{
        mouseOverMainBtn(idx){  //전달인자가 보낸 것이 매개변수  ==>  전달인자 idx => 매개변수  i 
