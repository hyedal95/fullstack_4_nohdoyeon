 ...
 */
// 1. 인터페이스 선언
interface Grade인터페이스2 {
	HashMap getGrade(String name, int jumsoo);
	
	
	
}
// 2. 클래스선언 구현(implements) 인터페이스 
class Grade평가클래스조건문2 implements Grade인터페이스2{
	...
	@Override
	public HashMap getGrade(String name, int jumsoo) {
		...
		return  map; // 이름: 이수신 , 학점: A+, 문자열 저장
	}
}


public class S03_클래스_상속_인터페이스_성적평가_해시맵사용 {

	public static void main(String[] args) {
		Grade평가클래스조건문2 a =new Grade평가클래스조건문2();
		
		
		System.out.println();
		//System.out.println(a.map);
		a.getGrade("이순신", 95);
		System.out.printf("이름 : %s, 학점 : %s\n" , a.map.get("이름"), a.map.get("학점") );
		a.getGrade("조지현", 78);
		System.out.printf("이름 : %s, 학점 : %s\n" , a.map.get("이름"), a.map.get("학점") );
		a.getGrade("이수정", 85);
		System.out.printf("이름 : %s, 학점 : %s\n" , a.map.get("이름"), a.map.get("학점") );
