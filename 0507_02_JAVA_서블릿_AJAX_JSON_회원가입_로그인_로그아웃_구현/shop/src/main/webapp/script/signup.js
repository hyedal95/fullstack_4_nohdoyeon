...
(($)=>{
    ...
    function list(res){
	    ...
	    arr.map((item,idx)=>{
			list += `<tr data-key=${item.아이디} data-pw=${item.비밀번호}>`;
			list += `<td>${idx+1}</td>`;
			list += `<td>${item.아이디}</td>`;
	...
	*/
	...
	$(document).on('mouseenter','.delete-btn', function(){
		$(this).on({
			click(e){
			e.preventDefault();
			const userId = $(this).parents('tr').data('key');
			const userPw = $(this).parents('tr').data('pw');
			// 	Query String ? key1 = value1 & key2 = value2;
			location.href =`user_delete_action.jsp?userId=${userId}&userPw=${userPw}`;
